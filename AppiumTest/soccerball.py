...
coords = {
    ...
    'select_session': (500, 700),        # 选择场次
    'select_ticket': (560, 1220),         # 选择票档
    'confirm_btn': (560, 2260),           # 确定按钮
    'expand_map': (995, 930),           # 扩大地图
    'select_seat': (650, 1001),           # 选择座位
    'confirm_select': (840, 2250),           # 确认选择
...
def fast_purchase():
    ...
    wait_for(0.8)
    adb_tap(*coords['expand_map'])
    wait_for(0.8)
    adb_tap(*coords['select_seat'])
    wait_for(0.1)
    adb_tap(*coords['confirm_select'])
    wait_for(0.8)
